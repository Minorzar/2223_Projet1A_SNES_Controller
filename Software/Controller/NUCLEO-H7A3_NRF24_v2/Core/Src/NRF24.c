...
void NRF24_ShowMemory(){
	uint8_t value ;
	uint8_t tab[26] = {REG_CONFIG,REG_EN_AA,REG_EN_RXADDR,REG_SETUP_AW,REG_SETUP_RETR,REG_RF_CH,REG_RF_SETUP,REG_STATUS,REG_OBSERVE_TX,
			REG_RPD,REG_RX_ADDR_P0,REG_RX_ADDR_P1,REG_RX_ADDR_P2,REG_RX_ADDR_P3,REG_RX_ADDR_P4,REG_RX_ADDR_P5,REG_TX_ADDR,REG_RX_PW_P0,
			REG_RX_PW_P1,REG_RX_PW_P2,REG_RX_PW_P3,REG_RX_PW_P4,REG_RX_PW_P5,REG_FIFO_STATUS,REG_DYNPD,REG_FEATURE};
	char uartTxBuf[100];
	/*for(int i = 0 ; i<26 ; i++){
		NRF24_read_register(tab[i]);
		storage[i]=data ;
	}*/

	for(int i = 0 ; i<26 ; i++){

		value = NRF24_read_register(tab[i]);
		sprintf(uartTxBuf, "Mem 0x%2x : 0x%2x \r\n",tab[i],value);
